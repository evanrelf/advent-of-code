...
impl Submarine {
    fn execute(&mut self, command: &Command) {
        match command {
            Command::Forward(distance) => self.horizontal_position += *distance as i32,
            Command::Down(distance) => self.depth += *distance as i32,
            Command::Up(distance) => self.depth -= *distance as i32,
        }
    }
}
