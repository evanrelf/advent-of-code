...
pub fn solve(mut input: &str) -> anyhow::Result<usize> {
    ...
    let to_minimum = |game: Game| CubeSet {
        red: game.cubes.iter().map(|c| c.red).max().unwrap_or(0),
        green: game.cubes.iter().map(|c| c.green).max().unwrap_or(0),
        blue: game.cubes.iter().map(|c| c.blue).max().unwrap_or(0),
    };

    let power = |cubes: CubeSet| cubes.red * cubes.green * cubes.blue;

    let solution = games.into_iter().map(to_minimum).map(power).sum();
...
mod tests {
    ...
    fn test() {
        ...
        "};
        assert_eq!(solve(input).unwrap(), 2286);
    }
}
