...
use winnow::{
    ...
    combinator::{delimited, repeat, separated, separated_pair, terminated},
    prelude::*,
};
...
fn almanac(input: &mut &str) -> PResult<Almanac> {
    ...
    let seeds =
        delimited("seeds: ", separated(1.., seed_range, " "), line_endings2).parse_next(input)?;
...
impl SeedRange {
    fn seeds(&self) -> impl ParallelIterator<Item = Seed> {
        let range = self.start..(self.start + self.length);
        range.into_par_iter().map(Seed::from)
    }
}

fn seed_range(input: &mut &str) -> PResult<SeedRange> {
    let (start, length) = separated_pair(usize, " ", usize).parse_next(input)?;
    Ok(SeedRange { start, length })
}
...
mod tests {
    ...
    #[test]
    fn test_sample() {
        assert_eq!(solve(SAMPLE).unwrap(), 46);
    }
}
